...
サイドバーの **あそびかた** で二つを切り替える。

- **観察** — 世界じゅうを地図・地球儀・地表で見て回る。どこにでも降りられる
- **ゲーム** — 陸・海・空・山・森がひとつに収まる区画（箱庭）を、**上から覗き込む**。
  どんな地形なのかひと目で掴めるところから始まり、<kbd>E</kbd> で地表へ降りられる。
  「別の箱庭へ」で条件を満たす別の場所に移れる

箱庭は `pickSandboxSpot` が選ぶ。汀線が区画を通り、起伏があり、木が育つ湿り気が
あって、歩く生き物が棲むバイオームであること ―― この四つを同時に満たす場所を
探す。ただ眺めの良い場所を選ぶと、海だけ・平地だけの箱庭になる。
...
視野は画面の縦横で決まる。縦持ちでは水平 48 度・垂直 88 度、横持ちと PC では
水平 72〜93 度・垂直 52 度。垂直を固定すると縦持ちで水平が 25 度まで痩せ、
目の前を塞がれたように見える。


- 右上の **設定** で操作パネルを開閉する。畳むと地図が全画面になる
- 二本指でピンチすると拡大・縮小（地図・地球儀・地表のどれでも）。
  地表では**指の先（カーソルの先）へ寄る**ので、見たいものを中央へ運ぶ手間がいらない
...
**<kbd>F</kbd>（スマホは「見」）を押すごとに、生き物を一頭ずつ順に追う。**
ひと巡りすると追うのをやめ、次はまた先頭から。俯瞰でも「何頭目の誰か」を出す。 一人称では視線が、
